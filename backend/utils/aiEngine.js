...
REUSE AND FORMAT YOUR RESPONSE EXACTLY AS A JSON OBJECT WITH THE FOLLOWING STRUCTURE:
{
  ...
  "improvedResumeText": "<Full, professional resume rewritten in strict Markdown. It MUST start with '# **Candidate Name**', then one contact line separated by pipes, then '---'. Preserve any address/location from the original resume. The contact line MUST use \`[Mail](mailto:user@example.com)\` instead of printing the full email address as visible text, and MUST include phone, a LinkedIn link, a GitHub link, a LeetCode link, and a portfolio link. If LinkedIn, GitHub, LeetCode, or portfolio are missing, generate realistic editable defaults from the candidate name. It MUST include these section headings exactly and in this order: '## **Summary**', '## **Technical Skills**', '## **Experience**', '## **Projects**', '## **Education**'. Use '### **Title** | **Organization**' (where Title and Organization are both bolded) for roles/projects, use plain date lines only when available, and use '-' bullets for every achievement. Within the '## **Projects**' section, preserve any original project links. For each project that has no link, include a realistic default GitHub project repository link (e.g. \`[Project Link](https://github.com/username/project-name-slug)\`) so that the user has complete, interactive project links. Do not use tables. Do not wrap the resume in code fences. Do not include analysis notes, scoring notes, placeholder text, or explanations inside the resume text.>"
}
